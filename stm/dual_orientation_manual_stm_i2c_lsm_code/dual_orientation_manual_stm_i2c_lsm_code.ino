...
void loop() {
  ...
  while (Wire.available())
  {
    ...
    int comma_6 = st.indexOf(',', comma_5 + 1);
    //    Serial.println();

    int ori, hrd_brk, soft_brk, anti_clo, clo, pwm, dir ;
    ...
    ori = (st.substring(comma_6 + 1).toInt());
    String  pwm_l = (st.substring(comma_4 + 1, comma_5));
    String  dir_l = (st.substring(comma_5 + 1, comma_6));
